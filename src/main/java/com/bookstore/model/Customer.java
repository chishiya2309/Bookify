...
public class Customer extends User {
    ...
    @Pattern(regexp = "^(\\+84|0)[0-9]{9}$", message = "Số điện thoại không hợp lệ. Vui lòng sử dụng 10 số (VD: 0912345678)")
    @Size(min = 10, max = 20, message = "Độ dài số điện thoại phải từ 10 đến 20 ký tự")
    @Column(name = "phone_number", length = 20, nullable = false)
    private String phoneNumber;

    @Column(name = "register_date", nullable = false)
    private LocalDateTime registerDate = VietnamTimeConfig.now();
    ...
    public Customer() {
        super();
        this.registerDate = VietnamTimeConfig.now();
    }
