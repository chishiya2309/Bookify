...
public class ShoppingCartServices {
     ...
     * 
     * <p>
     * Phương thức này kết hợp logic của {@link #getCartByCustomer(Customer)} và
     ...
     * cơ sở dữ liệu nếu khách hàng chưa có giỏ hàng.
     * </p>
     * 
     * <p>
     ...
     * </ul>
     * 
     * @param customer Đối tượng khách hàng cần lấy hoặc tạo giỏ hàng. Không được
     *                 null
     *                 và customer.getUserId() không được null.
     * @return Giỏ hàng hiện có hoặc mới tạo của khách hàng, đã được lưu vào cơ sở
     *         dữ liệu.
     ...
     * 
     *                              /**
     *                              Lấy giỏ hàng hiện có của khách hàng hoặc tạo mới
     *                              nếu chưa tồn tại.
     * 
     *                              <p>
     *                              Phương thức này kết hợp logic của
     *                              {@link #getCartByCustomer(Customer)} và
     *                              {@link #createCart(Customer)} để đảm bảo rằng
     *                              luôn có một giỏ hàng hợp lệ
     *                              được trả về cho khách hàng. Nó sẽ tự động tạo
     *                              giỏ hàng mới và lưu vào
     *                              cơ sở dữ liệu nếu khách hàng chưa có giỏ hàng.
     *                              </p>
     * 
     *                              <p>
     ...
     *                              </p>
     *                              <ul>
     *                              <li>Sử dụng phương thức này khi cần đảm bảo
     *                              khách hàng luôn có giỏ hàng
     *                              (ví dụ: khi thêm sản phẩm, xem giỏ hàng, hoặc
     *                              merge giỏ hàng)</li>
     *                              <li>Sử dụng {@link #getCartByCustomer(Customer)}
     *                              khi chỉ cần kiểm tra xem
     *                              khách hàng có giỏ hàng hay không mà không muốn
     *                              tạo mới</li>
     *                              <li>Sử dụng {@link #createCart(Customer)} khi
     *                              cần tạo giỏ hàng mới một cách
     *                              rõ ràng (ví dụ: sau khi đơn hàng được hoàn
     *                              tất)</li>
     *                              </ul>
     * 
     * @param customer Đối tượng khách hàng cần lấy hoặc tạo giỏ hàng. Không được
    ...
    public MergeResult mergeGuestCartToUserCart(ShoppingCart guestCart, Customer customer) {
        ...
        // Lấy hoặc tạo giỏ hàng của user từ DB (sử dụng method mới để xử lý detached
        // entity)
    ...
    public static class MergeResult {
        ...
        public void addLimitedItem(String bookTitle, int requestedQty, int availableQty) {
            limitedItems.add(String.format(
                    "'%s': đã giới hạn đến %d cuốn (theo yêu cầu là %d, không đủ số lượng hàng trong kho)", bookTitle,
        ...
        public String getSummaryMessage() {
            ...
            if (!newItems.isEmpty()) {
                sb.append("Đã thêm vào sách mới:\n");
                newItems.forEach(msg -> sb.append("  • ").append(msg).append("\n"));
            }

            if (!mergedItems.isEmpty()) {
                sb.append("Đã cập nhật số lượng:\n");
                mergedItems.forEach(msg -> sb.append("  • ").append(msg).append("\n"));
            }

            if (!limitedItems.isEmpty()) {
                sb.append("Giới hạn số lượng trong kho:\n");
                limitedItems.forEach(msg -> sb.append("  • ").append(msg).append("\n"));
            }
